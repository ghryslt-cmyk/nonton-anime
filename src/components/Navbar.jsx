...
export default function Navbar() {
  ...
  return (
    <nav className="bg-slate-900 border-b border-teal-700/50 px-4 md:px-6 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          <Feather className="w-6 h-6 md:w-8 md:h-8 text-teal-400" />
          <span className="text-lg md:text-2xl font-bold bg-gradient-to-r from-teal-400 to-cyan-500 bg-clip-text text-transparent">
            WorldEnd Stream
          </span>
        </Link>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden mt-4 pb-4 border-t border-slate-700 pt-4">
          ...
          <div className="flex flex-col gap-2">
            <Link to="/" className="text-gray-300 hover:text-teal-400 transition py-2" onClick={() => setMobileMenuOpen(false)}>
              Home
            </Link>
            <Link to="/browse" className="text-gray-300 hover:text-teal-400 transition py-2" onClick={() => setMobileMenuOpen(false)}>
              Browse
            </Link>
            <Link to="/trending" className="text-gray-300 hover:text-teal-400 transition py-2" onClick={() => setMobileMenuOpen(false)}>
              Trending
            </Link>
            ...
            {user ? (
              <>
                <Link to="/profile" className="text-gray-300 hover:text-teal-400 transition py-2" onClick={() => setMobileMenuOpen(false)}>
                  Profile
                </Link>
                <button
                  onClick={() => {
              ...
              <Link to="/login" className="flex items-center gap-2 bg-teal-600 hover:bg-teal-700 px-4 py-2 rounded-lg transition w-fit" onClick={() => setMobileMenuOpen(false)}>
                <User className="w-5 h-5" />
                <span>Login</span>
              </Link>
            )}
